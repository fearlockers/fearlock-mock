...
-- 組織作成を許可するポリシー（認証済みユーザー）
DROP POLICY IF EXISTS "Users can create organizations" ON organizations;
CREATE POLICY "Users can create organizations" ON organizations
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- 組織作成後の表示を許可するポリシー
DROP POLICY IF EXISTS "Users can view created organizations" ON organizations;
CREATE POLICY "Users can view created organizations" ON organizations
  FOR SELECT USING (
...
-- 組織作成後のプロフィール更新を許可するポリシー
DROP POLICY IF EXISTS "Users can update profiles after organization creation" ON profiles;
CREATE POLICY "Users can update profiles after organization creation" ON profiles
  FOR UPDATE USING (
    id = auth.uid() OR 
    organization_id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
...
-- 組織作成後のプロフィール挿入を許可するポリシー
DROP POLICY IF EXISTS "Users can insert profiles after organization creation" ON profiles;
CREATE POLICY "Users can insert profiles after organization creation" ON profiles
...
-- 一時的にRLSを緩和して組織作成を許可（開発環境用）
-- 注意: 本番環境では適切なセキュリティポリシーを設定してください
DROP POLICY IF EXISTS "Temporary development policy" ON organizations;
CREATE POLICY "Temporary development policy" ON organizations
